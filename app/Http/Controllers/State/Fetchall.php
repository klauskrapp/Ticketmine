...
class Fetchall extends State
{
    ...
    {
        // set default query
        $sql    = 'SELECT state.*, project.name as project_name FROM state LEFT JOIN project on project.id = state.project_id';
        $filter = new Filter();



        // default order
        $orderQuery					= $filter->getOrderBy( $request->get('sorting'), ' ORDER BY state.id DESC, project_id DESC' );
        ...
        $mView	= View::make( 'state.fetchall', array(
            'rows'      => $arrItems
        ) );
