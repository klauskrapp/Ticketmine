<?php
return array(
    'load_more_entries'   => 'mehr laden...',
    'created'             => 'erstellt',
    'on'                  => 'am',
    'at'                => 'um',
    'name'              => 'Name',
    'assigned'              => 'Zugewiesen',
    'priority'              => 'Priorität',
    'actiontype'              => 'Vorgangstyp',
    'state'                 => 'Status',
    'deleted_ticket'        => 'löscht Ticket',
    'action_changed'        => 'ändert den Vorgangstyp von {from} auf {to}',
    'priority_changed'      => 'ändert Priorität von {from} auf {to}',
    'state_changed'      => 'ändert Status von {from} auf {to}',
    'assignees_changed'      => 'ändert Bearbeiter von {from} auf {to}',
    'author_changed'      => 'ändert Autor von {from} auf {to}',
    'follower_changed'      => 'ändert Beobachter von {from} auf {to}',
);
