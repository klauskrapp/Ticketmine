@foreach( $rows as $row )
    ...
    <tr class="align-middle {{\App\Helpers\Grid::getEvenOrOdd()}}">
        ...
        <td>
            <span class="btn btn-danger" onclick="deleteEntity('{{url('manage/delete/' . $row->id)}}', 'manage-index-table');">
                <svg class="icon">
                    <use xlink:href="{{get_core_ui_path()}}vendors/@coreui/icons/svg/free.svg#cil-trash"></use>
                </svg>
            </span>
            <a class="btn btn-info" href="{{url('manage/configure/' . $row->id)}}">
                <svg class="icon">
                    <use xlink:href="{{get_core_ui_path()}}vendors/@coreui/icons/svg/free.svg#cil-filter"></use>
                </svg>
            </a>
            <a class="btn btn-success me-2" href="{{url('manage/edit/' . $row->id)}}">
                <svg class="icon">
